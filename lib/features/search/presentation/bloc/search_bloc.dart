...
class SearchBloc extends Bloc<SearchEvent, SearchState> {
  ...
  SearchBloc({required this.fetchSearchResultUseCase})
  ...
  _onFetchSearchResultEvent(
      FetchSearchResultEvent event, Emitter<SearchState> emit) async {
    emit(const SearchLoading());
    final Either<Failure, List<SearchDataModel>> result =
        await fetchSearchResultUseCase.call(event.searchPhrase);
    result.fold((l) => emit(SearchError(message: l.toString())), (r) {
      emit(SearchLoaded(data: r));
    });
  }

  _onClearSearchResultEvent(
      ClearSearchResultEvent event, Emitter<SearchState> emit) {
